...
export class Controller {
	...
	*/
	...
	async handleInferallCallback(token: string, email: string | null, _state: string | null) {
		...
		if (email) {
			await this.stateManager.setSecret("inferall:email", email)
		}
		await this.stateManager.setSecret("inferall:token", token)
		...
		Logger.info(`InferAll: Signed in as ${email || "unknown"}`)
	}
