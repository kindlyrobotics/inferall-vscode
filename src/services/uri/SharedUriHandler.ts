	...
	public static async handleUri(url: string): Promise<boolean> {
		...
		try {
			switch (path) {
				...
				case "/auth/callback":
				case "/inferall": {
					// InferAll sign-in callback: receives kr_user_* token from inferall.ai
					const inferallToken = query.get("token")
					const inferallEmail = query.get("email")
					const inferallState = query.get("state")
					if (inferallToken) {
						await visibleWebview.controller.handleInferallCallback(inferallToken, inferallEmail, inferallState)
						return true
					}
					Logger.warn("SharedUriHandler: Missing token parameter for InferAll callback")
					return false
				}
				default:
